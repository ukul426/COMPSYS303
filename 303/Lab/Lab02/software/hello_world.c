...
int main()
{
	...
	while(1)
	{
		...
		if(lcd != NULL)//character LCD was open or not
		{
			//updates the LCD display when the button values change, preventing unnecessary LCD refresh
			if(uiButtonsValuePrevious != uiButtonsValue)
			{
				//ESC and CLEAR_LCD_STRING are commands to the LCD for clear the display.
				#define ESC 27
				#define CLEAR_LCD_STRING "[2J"
				fprintf(lcd, "%c%s", ESC, CLEAR_LCD_STRING);//write the characters/string to the LCD
				fprintf(lcd, "BUTTON VALUE: %d\n", uiButtonsValue);
			}
		}
	}
	fclose(lcd);//LCD is required once finish using the LCD
