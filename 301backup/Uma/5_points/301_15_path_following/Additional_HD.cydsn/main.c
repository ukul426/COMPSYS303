...
void turnLeft(){
    ...
    PWM_2_WriteCompare(31);
     while(abs(QuadDec_M2_GetCounter())<55){
        ;;
    }
    PWM_1_WriteCompare(50);
    PWM_2_WriteCompare(50);
...
void turnRight(){
    ...
    PWM_2_WriteCompare(69);
     while(abs(QuadDec_M1_GetCounter())<60){
        ;;
    }
    PWM_1_WriteCompare(50);
    PWM_2_WriteCompare(50);
...
void turn_180(){
    ...
    QuadDec_M2_SetCounter(0);
    reverse();
    while(abs(QuadDec_M2_GetCounter())<75){
        ;;
    }
    stop();
    ...
    for(int i=0;i<5;i++){
       ...
       getMovementArray(food_list[i][0],food_list[i][1], food_list[i+1][0],food_list[i+1][1],startDirectionArray[i]);
        ...
        for(int i=0; i<25;i++){//length should be how many movements there should be
           // if(i!=0){
            goStraight_cm(movementArray[i].distance-8);
            if (comp0_sum==0 || comp1_sum==0){
                goStraight_cm(5);//if not the first movement remove 5 cm as turn moves cart roughly 5 cm forward
            }else if(Sout_R_Read()==0){
                turnRight();
            }else if(Sout_L_Read()==0){
                turnLeft();
            }
           // }else{
