...
void comp_clk_Start(void) 
{
    ...
    comp_clk_CLKEN |= comp_clk_CLKEN_MASK;
	comp_clk_CLKSTBY |= comp_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: comp_clk_Stop
********************************************************************************
*
* Summary:
...
void comp_clk_Stop(void) 
{
    ...
    comp_clk_CLKEN &= (uint8)(~comp_clk_CLKEN_MASK);
	comp_clk_CLKSTBY &= (uint8)(~comp_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: comp_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((comp_clk_CLKEN & comp_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = comp_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = comp_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* comp_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(comp_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(comp_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: comp_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: comp_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = comp_clk_GetSourceRegister();
    uint16 oldDivider = comp_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = comp_clk_CLKEN & comp_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((comp_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: comp_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(comp_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: comp_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    comp_clk_MOD_SRC |= modeBitMask & (uint8)comp_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: comp_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    comp_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(comp_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: comp_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return comp_clk_MOD_SRC & (uint8)(comp_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: comp_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = comp_clk_GetDividerRegister();
    uint8 oldSrc = comp_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        comp_clk_MOD_SRC |= CYCLK_SSS;
        comp_clk_MOD_SRC =
            (comp_clk_MOD_SRC & (uint8)(~comp_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: comp_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return comp_clk_MOD_SRC & comp_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: comp_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    comp_clk_PHASE = clkPhase & comp_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: comp_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return comp_clk_PHASE & comp_clk_PHASE_MASK;
}
